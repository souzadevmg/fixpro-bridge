...
COMMANDS: dict[str, BridgeCommand] = {
    "tailscale": BridgeCommand("Tailscale/IP", ["sh", "-lc", "command -v tailscale >/dev/null 2>&1 && tailscale ip -4 || true; command -v ip >/dev/null 2>&1 && ip -4 addr show || true; command -v ifconfig >/dev/null 2>&1 && ifconfig || true"]),
    "network": BridgeCommand("Rede Android", ["sh", "-lc", "command -v ip >/dev/null 2>&1 && { ip -4 addr show; ip route; } || { command -v ifconfig >/dev/null 2>&1 && ifconfig || true; }"]),
